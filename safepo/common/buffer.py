...
class VectorizedOnPolicyBuffer_2constraints:
    """
    A buffer for storing vectorized on-policy data for reinforcement learning.
    ...
    Args:
        ...
        gamma (float, optional): The discount factor for rewards. Defaults to 0.99.
        lam (float, optional): The lambda parameter for GAE computation. Defaults to 0.95.
        lam_c1 (float, optional): The lambda parameter for cost1 GAE computation. Defaults to 0.95.
        lam_c2 (float, optional): The lambda parameter for cost2 GAE computation. Defaults to 0.95.
        standardized_adv_r (bool, optional): Whether to standardize advantage rewards. Defaults to True.
        standardized_adv_c1 (bool, optional): Whether to standardize advantage costs1. Defaults to True.
        standardized_adv_c2 (bool, optional): Whether to standardize advantage costs2. Defaults to True.
        device (torch.device, optional): The device to store tensors on. Defaults to "cpu".
        num_envs (int, optional): The number of parallel environments. Defaults to 1.
    """
    def __init__(
        ...
        lam_c2: float=0.95,
        standardized_adv_r: bool = True,
        standardized_adv_c1: bool = True,
        standardized_adv_c2: bool = True,
        device: torch.device = "cpu",
        num_envs: int = 1,
    ) -> None:
        self.buffers: list[dict[str, torch.tensor]] = [
            {
                ...
                "reward": torch.zeros(size, dtype=torch.float32, device=device),
                "cost1": torch.zeros(size, dtype=torch.float32, device=device),
                "cost2": torch.zeros(size, dtype=torch.float32, device=device),
                "done": torch.zeros(size, dtype=torch.float32, device=device),
                "value_r": torch.zeros(size, dtype=torch.float32, device=device),
                "value_c1": torch.zeros(size, dtype=torch.float32, device=device),
                "value_c2": torch.zeros(size, dtype=torch.float32, device=device),
                "adv_r": torch.zeros(size, dtype=torch.float32, device=device),
                "adv_c1": torch.zeros(size, dtype=torch.float32, device=device),
                "adv_c2": torch.zeros(size, dtype=torch.float32, device=device),
                "target_value_r": torch.zeros(size, dtype=torch.float32, device=device),
                "target_value_c1": torch.zeros(size, dtype=torch.float32, device=device),
                "target_value_c2": torch.zeros(size, dtype=torch.float32, device=device),
                "log_prob": torch.zeros(size, dtype=torch.float32, device=device),
            }
            for _ in range(num_envs)
        ]
        ...
        self._lam_c2 = lam_c2
        self._standardized_adv_r = standardized_adv_r
        self._standardized_adv_c1 = standardized_adv_c1
        self._standardized_adv_c2 = standardized_adv_c2
        self.ptr_list = [0] * num_envs
        self.path_start_idx_list = [0] * num_envs
        self._device = device
        self.num_envs = num_envs
    ...
    def finish_path(
        self,
        last_value_r: torch.Tensor | None = None,
        last_value_c1: torch.Tensor | None = None,
        last_value_c2: torch.Tensor | None = None,
        idx: int = 0,
    ) -> None:
        """
        Finalize the trajectory path and compute advantages and value targets.
        ...
        Args:
            last_value_r (torch.Tensor, optional): The last value estimate for rewards. Defaults to None.
            last_value_c1 (torch.Tensor, optional): The last value estimate for costs1. Defaults to None.
            last_value_c2 (torch.Tensor, optional): The last value estimate for costs2. Defaults to None.
            idx (int, optional): Index of the environment. Defaults to 0.
        """
        if last_value_r is None:
            last_value_r = torch.zeros(1, device=self._device)
        if last_value_c1 is None:
            last_value_c1 = torch.zeros(1, device=self._device)
        if last_value_c2 is None:
            last_value_c2 = torch.zeros(1, device=self._device)
        path_slice = slice(self.path_start_idx_list[idx], self.ptr_list[idx])
        last_value_r = last_value_r.to(self._device)
        last_value_c1 = last_value_c1.to(self._device)
        last_value_c2 = last_value_c2.to(self._device)
        rewards = torch.cat([self.buffers[idx]["reward"][path_slice], last_value_r])
        costs1 = torch.cat([self.buffers[idx]["cost1"][path_slice], last_value_c1])
        costs2 = torch.cat([self.buffers[idx]["cost2"][path_slice], last_value_c2])
        values_r = torch.cat([self.buffers[idx]["value_r"][path_slice], last_value_r])
        values_c1 = torch.cat([self.buffers[idx]["value_c1"][path_slice], last_value_c1])
        values_c2 = torch.cat([self.buffers[idx]["value_c2"][path_slice], last_value_c2])
        ...
        adv_c1, target_value_c1 = calculate_adv_and_value_targets(
            values_c1,
            costs1,
            lam=self._lam_c1,
            gamma=self._gamma,
        )
        adv_c2, target_value_c2 = calculate_adv_and_value_targets(
            values_c2,
            costs2,
            lam=self._lam_c2,
            gamma=self._gamma,
        ...
        self.buffers[idx]["adv_r"][path_slice] = adv_r
        self.buffers[idx]["adv_c1"][path_slice] = adv_c1
        self.buffers[idx]["adv_c2"][path_slice] = adv_c2
        self.buffers[idx]["target_value_r"][path_slice] = target_value_r
        self.buffers[idx]["target_value_c1"][path_slice] = target_value_c1
        self.buffers[idx]["target_value_c2"][path_slice] = target_value_c2
    ...
    def get(self) -> dict[str, torch.Tensor]:
        ...
        adv_mean = data["adv_r"].mean()
        adv_std = data["adv_r"].std()
        cadv1_mean = data["adv_c1"].mean()
        cadv2_mean = data["adv_c2"].mean()
        if self._standardized_adv_r:
            data["adv_r"] = (data["adv_r"] - adv_mean) / (adv_std + 1e-8)
        if self._standardized_adv_c1:
            data["adv_c1"] = data["adv_c1"] - cadv1_mean
        if self._standardized_adv_c2:
            data["adv_c2"] = data["adv_c2"] - cadv2_mean
        self.ptr_list = [0] * self.num_envs
        self.path_start_idx_list = [0] * self.num_envs
...
class GCVectorizedOnPolicyBuffer:
    """
    A buffer for storing vectorized on-policy data for reinforcement learning.
        ...
        self.buffers: list[dict[str, torch.tensor]] = [
            {
                ...
                "value_r": torch.zeros(size, dtype=torch.float32, device=device),
                "value_c": torch.zeros(size, dtype=torch.float32, device=device),
                "adv_lag": torch.zeros(size, dtype=torch.float32, device=device),
                "adv_r": torch.zeros(size, dtype=torch.float32, device=device),
                "adv_c": torch.zeros(size, dtype=torch.float32, device=device),
                "target_value_r": torch.zeros(size, dtype=torch.float32, device=device),
                "target_value_c": torch.zeros(size, dtype=torch.float32, device=device),
                "log_prob_r": torch.zeros(size, dtype=torch.float32, device=device),
                "log_prob_c": torch.zeros(size, dtype=torch.float32, device=device),
            }
            for _ in range(num_envs)
        ]
        self._gamma = gamma
    ...
    def get(self) -> dict[str, torch.Tensor]:
        ...
        for buffer in self.buffers[1:]:
            for k, v in buffer.items():
                data_pre[k].append(v)
        data = {k: torch.cat(v, dim=0) for k, v in data_pre.items()}
        adv_r_mean = data["adv_r"].mean()
        adv_r_std = data["adv_r"].std()
        adv_c_mean = data["adv_c"].mean()
        adv_c_std = data["adv_c"].std()
        adv_lag_mean = data["adv_lag"].mean()
        adv_lag_std = data["adv_lag"].std()

        if self._standardized_adv_r:
            data["adv_r"] = (data["adv_r"] - adv_r_mean) / (adv_r_std + 1e-8)
            data["adv_c"] = (data["adv_c"] - adv_c_mean) / (adv_c_std + 1e-8)
            data["adv_lag"] = (data["adv_lag"] - adv_lag_mean) / (adv_lag_std + 1e-8)
...
class MultiObjectiveVectorizedOnPolicyBuffer:
    """
    用于存储多目标向量化策略数据的缓冲区。

    参数:
        obs_space (gymnasium.Space): 观测空间。
        act_space (gymnasium.Space): 动作空间。
        size (int): 缓冲区的最大大小。
        gamma (float, 可选): 奖励的折扣因子。默认为 0.99。
        lam (float, 可选): GAE 计算的 lambda 参数。默认为 0.95。
        num_objectives (int, 可选): 目标的数量。默认为 2。
        standardized_adv (bool, 可选): 是否对优势进行标准化。默认为 True。
        device (torch.device, 可选): 存储张量的设备。默认为 "cpu"。
        num_envs (int, 可选): 并行环境的数量。默认为 1。
    """
    def __init__(
        self,
        obs_space,
        ...
        self.buffers: list[dict[str, torch.tensor]] = [
            {
                ...
                "rewards": torch.zeros(
                    (size, num_objectives), dtype=torch.float32, device=device
                ),
                "done": torch.zeros(size, dtype=torch.float32, device=device),
                "values": torch.zeros(
                    (size, num_objectives), dtype=torch.float32, device=device
        ...
        self._gamma = gamma
        self._lam = lam
        self._standardized_adv = standardized_adv
        self.ptr_list = [0] * num_envs
        self.path_start_idx_list = [0] * num_envs
        self._device = device
        self.num_envs = num_envs
    ...
    def store(self, **data: torch.Tensor) -> None:
        ...
        参数:
            **data: 指定要存储的数据张量的关键字参数。
        """
        for i, buffer in enumerate(self.buffers):
            assert self.ptr_list[i] < buffer["obs"].shape[0], "缓冲区溢出"
            for key, value in data.items():
                if key == "rewards" or key == "values" or key == "advs" or key == "target_values":
                    buffer[key][self.ptr_list[i]] = value[i]
                else:
                    buffer[key][self.ptr_list[i]] = value[i]
            self.ptr_list[i] += 1
    ...
    def finish_path(
        ...
        参数:
            last_values (torch.Tensor, 可选): 奖励的最后值估计。默认为 None。
            idx (int, 可选): 环境的索引。默认为 0。
        """
        if last_values is None:
            last_values = torch.zeros(self.num_objectives, device=self._device)
        path_slice = slice(self.path_start_idx_list[idx], self.ptr_list[idx])
        last_values = last_values.to(self._device)
        rewards = torch.cat([self.buffers[idx]["rewards"][path_slice], last_values.unsqueeze(0)], dim=0)
        values = torch.cat([self.buffers[idx]["values"][path_slice], last_values.unsqueeze(0)], dim=0)

        for obj_idx in range(self.num_objectives):
            adv, target_value = self.calculate_adv_and_value_targets(
                values[:, obj_idx],
                rewards[:, obj_idx],
                lam=self._lam,
                gamma=self._gamma,
            )
            self.buffers[idx]["advs"][path_slice, obj_idx] = adv
            self.buffers[idx]["target_values"][path_slice, obj_idx] = target_value
    ...
    def get(self) -> dict[str, torch.Tensor]:
        ...
        返回:
            dict[str, torch.Tensor]: 包含收集的数据张量的字典。
        """
        data_pre = {k: [v] for k, v in self.buffers[0].items()}
        for buffer in self.buffers[1:]:
            for k, v in buffer.items():
        ...
        for obj_idx in range(self.num_objectives):
            adv_mean = data["advs"][:, obj_idx].mean()
            adv_std = data["advs"][:, obj_idx].std()
            if self._standardized_adv:
                data["advs"][:, obj_idx] = (data["advs"][:, obj_idx] - adv_mean) / (adv_std + 1e-8)
    ...
    def calculate_adv_and_value_targets(
        ...
        参数:
            values (torch.Tensor): 值估计。
            rewards (torch.Tensor): 奖励。
            lam (float): GAE 计算的 lambda 参数。
            gamma (float): 折扣因子。

        返回:
            tuple[torch.Tensor, torch.Tensor]: 优势和目标值。
        """
        deltas = rewards[:-1] + gamma * values[1:] - values[:-1]
        adv = self.discount_cumsum(deltas, gamma * lam)
        target_value = adv + values[:-1]
        return adv, target_value

    def discount_cumsum(self, vector_x: torch.Tensor, discount: float) -> torch.Tensor:
        """
        计算张量沿第一维的折扣累积和。

        参数:
            vector_x (torch.Tensor): 输入张量，形状为 `(length, ...)`。
            discount (float): 未来值的折扣因子。

        返回:
            torch.Tensor: 包含 `vector_x` 的折扣累积和的张量。
        """
        length = vector_x.shape[0]
        vector_x = vector_x.type(torch.float64)
        cumsum = vector_x[-1]
