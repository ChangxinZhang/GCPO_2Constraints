...
class SafeNormalizeObservation(NormalizeObservation):
    ...
    def step(self, action):
        ...
        obs, rews, costs1, costs2, terminateds, truncateds, infos = self.env.step(action)
        obs = self.normalize(obs) if self.is_vector_env else self.normalize(np.array([obs]))[0]
        return obs, rews, costs1, costs2, terminateds, truncateds, infos
...
try:
    class GymnasiumIsaacEnv(VecTaskPython):
        ...
        def step(self, action):
            ...
            obs, rews, costs1, costs2, terminated, infos = super().step(action)
            truncated = terminated
            return obs, rews, costs1, costs2, terminated, truncated, infos
