...
def main(args, cfg_env=None):
    ...
    for epoch in range(epochs):
        ...
        step_direction = (
            torch.sqrt(2 * config['target_kl'] / (q + 1e-8)) * H_inv_g
            - torch.clamp_min(
                (torch.sqrt(2 * config['target_kl'] / q) * r + ep_costs) / s,
        ...
        kl = torch.zeros(1)
        for step in range(PCPO_SEARCHING_STEPS):
