...
class ShadowHandOver_Safe_finger(BaseTask):
    def __init__(
        ...
        self.num_obs_dict = {'openai': 42, 'full_no_vel': 77, 'full': 157, 'full_state': 398}
    ...
    def compute_cost1(self):
        actions = self.actions.clone()

        self.cost1_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.float)

        self.cost1_buf = torch.where(
            actions[:, 2] < -0.5, torch.ones_like(self.cost1_buf), self.cost1_buf
        )
        self.cost1_buf = torch.where(
            actions[:, 2] > 0.5, torch.ones_like(self.cost1_buf), self.cost1_buf
        )

        return self.cost1_buf
    
    def compute_cost2(self):
        actions = self.actions.clone()

        self.cost2_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.float)


        self.cost2_buf = torch.where(
            actions[:, 3] < -0.5, torch.ones_like(self.cost2_buf), self.cost2_buf
        )
        self.cost2_buf = torch.where(
            actions[:, 3] > 0.5, torch.ones_like(self.cost2_buf), self.cost2_buf
        )

        return self.cost2_buf
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        rand_delta = (
            delta_min + (delta_max - delta_min) * rand_floats[:, 5 : 5 + self.num_shadow_hand_dofs]
        )
